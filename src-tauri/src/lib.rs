...
fn extract_word_text(filename: String, contents_base64: String) -> Result<String, String> {
    ...
    let bytes = STANDARD
        ...
        .map_err(|_| "无法读取 Word 文件内容。".to_string())?;
    if bytes.len() > 20 * 1024 * 1024 {
        return Err("Word 文件超过 20 MB，请拆分后再导入。".into());
    }
    let stamp = SystemTime::now()
        .duration_since(UNIX_EPOCH)
        .map_err(|_| "无法创建临时导入文件。".to_string())?
        .as_nanos();
    let extension = if lower_name.ends_with(".docx") {
        "docx"
    } else {
        "doc"
    };
    let path = std::env::temp_dir().join(format!(
        "inkstone-import-{}-{stamp}.{extension}",
        std::process::id()
    ));
    ...
    let text = String::from_utf8_lossy(&output.stdout)
        ...
        .to_string();
    if text.is_empty() {
        return Err("Word 文档中没有可导入的文本。".into());
    }
    Ok(text)
}
...
async fn web_search(query: String) -> Result<Vec<WebSearchResult>, String> {
    ...
    if query.chars().count() > 180 {
        return Err("搜索词过长，请控制在 180 个字符以内。".into());
    }
    let output = Command::new("/usr/bin/curl")
        ...
        .output()
        .map_err(|error| format!("无法启动网络搜索：{error}"))?;
    if !output.status.success() {
        return Err(format!(
            "网络搜索失败：{}",
            String::from_utf8_lossy(&output.stderr).trim()
    ...
    let body =
        String::from_utf8(output.stdout).map_err(|_| "搜索服务返回了无效内容。".to_string())?;
    let result_re = Regex::new(r#"(?s)<li[^>]*class=\"b_algo\"[^>]*>(.*?)(?=<li[^>]*class=\"b_algo\"|<li[^>]*class=\"b_pag\"|$)"#).map_err(|_| "无法解析搜索结果。".to_string())?;
    let title_re = Regex::new(r#"(?s)<h2[^>]*>\s*<a[^>]*href=\"([^\"]+)\"[^>]*>(.*?)</a>\s*</h2>"#)
        .map_err(|_| "无法解析搜索标题。".to_string())?;
    let snippet_re =
        Regex::new(r#"(?s)<p[^>]*>(.*?)</p>"#).map_err(|_| "无法解析搜索摘要。".to_string())?;
    ...
    for capture in result_re.captures_iter(&body).take(8) {
        let item = capture
            ...
            .unwrap_or_default();
        let Some(title_capture) = title_re.captures(item) else {
            continue;
        };
        let url = decode_html(
            title_capture
        ...
        let snippet = snippet_re
            ...
            .map(|match_| compact_text(strip_html(match_.as_str()), 300))
            .unwrap_or_default();
...
pub fn run() {
    tauri::Builder::default()
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            web_search,
            extract_word_text,
            native_store::load_native_store,
            native_store::migrate_web_store,
            native_store::put_native_entry,
            native_store::remove_native_entry,
            native_store::save_native_chapter
        ])
