...
import { BookOpen, CaretDown, Check, CheckCircle, ClockCounterClockwise, Compass, Export, FileArrowUp, FolderOpen, GearSix, NotePencil, Plus, Sparkle, UsersThree, X } from "@phosphor-icons/react";
...
import { readActiveProjectId, readChapterHistory, readEditorPreferences, readLibrary, readModelConfig, readProjects, readSavedChapters, readSkills, readStoryAnalysis, saveActiveProjectId, saveChapterVersion, saveEditorPreferences, saveModelConfig, saveProjects, saveSavedChapters, saveSkills, saveStoryAnalysis } from "./services/storage";
import { availableSkills } from "./services/skills";
import type { Chapter, ChapterVersion, EditorPreferences, ModelConfig, NovelProject, SavedChapter, StoryAnalysis, View, WritingSkill } from "./types/story";
import "./App.css";
...
function App() {
  ...
  const selectProject = (projectId: string) => {
    ...
    setSavedChapters(readSavedChapters(projectId)); setStoryAnalysis(readStoryAnalysis(projectId)); setSkills(readSkills(projectId)); setEditorPreferences(readEditorPreferences(projectId)); setView("write");
  };
