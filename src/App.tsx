...
import { BookOpen, CaretDown, Check, CheckCircle, ClockCounterClockwise, Compass, Export, FolderOpen, GearSix, NotePencil, Plus, UsersThree, X } from "@phosphor-icons/react";
...
import { readActiveProjectId, readModelConfig, readProjects, readSavedChapters, readStoryAnalysis, saveActiveProjectId, saveModelConfig, saveProjects, saveSavedChapters, saveStoryAnalysis } from "./services/storage";
import type { Chapter, ModelConfig, NovelProject, SavedChapter, StoryAnalysis, View } from "./types/story";
import "./App.css";
...
function App() {
  ...
  const selectProject = (projectId: string) => {
    ...
    setSavedChapters(readSavedChapters(projectId)); setStoryAnalysis(readStoryAnalysis(projectId)); setView("write");
  };
  ...
  const generateContinuation = async () => {
    ...
    try {
      ...
      const continuation = await generateNovelContinuation(modelConfig, draft); setDraft((current) => `${current}\n\n${continuation}`); setNotice("已将模型续写插入正文，可继续编辑或撤销。");
  ...
  const saveDraft = () => {
    ...
    saveSavedChapters(activeProject.id, nextSaved); setSavedChapters(nextSaved); updateChapters(activeProject.chapters.map((chapter) => chapter.id === activeChapter ? { ...chapter, words: wordCount.toLocaleString(), status: "ready" } : chapter));
