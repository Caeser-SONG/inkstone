...
import { WritingSessionSummary } from "./components/WritingSessionSummary";
...
import { analyzeSavedChapters, fallbackAnalysis } from "./services/storyAgent";
import { initializePersistentStorage, readActiveProjectId, readActiveWritingSession, readChapterGuide, readChapterHistory, readEditorPreferences, readLibrary, readModelConfig, readProjects, readSavedChapters, readSkills, readStoryAnalysis, readWritingSessions, saveActiveProjectId, saveActiveWritingSession, saveChapterGuide, saveChapterVersion, saveEditorPreferences, saveModelConfig, saveProjects, saveSavedChapter, saveSavedChapters, saveSkills, saveStoryAnalysis, saveWritingSessions } from "./services/storage";
import type { StorageLocation } from "./services/nativeStore";
import { availableSkills } from "./services/skills";
import type { ActiveWritingSession, Chapter, ChapterGuide, ChapterVersion, EditorPreferences, ModelConfig, NovelProject, SavedChapter, StoryAnalysis, StoryDecision, StoryEvidence, StoryMemoryStatus, View, WorkshopRound, WritingSession, WritingSkill } from "./types/story";
import "./App.css";
...
function emptyChapterGuide(chapterId: number): ChapterGuide {
  return { chapterId, goal: "", conflict: "", hook: "", updatedAt: new Date().toISOString() };
}
...
function App() {
  ...
  const [chapterGuide, setChapterGuide] = useState<ChapterGuide>(() => readChapterGuide(initialProjectId, activeProject?.chapters[0]?.id || 0) || emptyChapterGuide(activeProject?.chapters[0]?.id || 0));
  ...
  const [repositoryLocation, setRepositoryLocation] = useState<StorageLocation | null>(null);
  ...
  useEffect(() => {
    let cancelled = false;
    void initializePersistentStorage().then((location) => {
      if (cancelled) return;
      const nextProjects = readProjects();
      const nextProjectId = readActiveProjectId(nextProjects);
      const nextProject = nextProjects.find((project) => project.id === nextProjectId) || nextProjects[0];
      saveProjects(nextProjects);
      setProjects(nextProjects); setActiveProjectId(nextProjectId); setActiveChapter(nextProject?.chapters[0]?.id || 0);
      setDraft(nextProject?.chapters[0] ? (readSavedChapters(nextProject.id).find((chapter) => chapter.id === nextProject.chapters[0].id)?.content || (nextProject.id === "demo-project" ? initialDraft : "")) : "");
      setSavedChapters(readSavedChapters(nextProjectId)); setStoryAnalysis(readStoryAnalysis(nextProjectId)); setChapterGuide(readChapterGuide(nextProjectId, nextProject?.chapters[0]?.id || 0) || emptyChapterGuide(nextProject?.chapters[0]?.id || 0)); setSkills(readSkills(nextProjectId)); setEditorPreferences(readEditorPreferences(nextProjectId)); setActiveWritingSession(readActiveWritingSession(nextProjectId)); setWritingSessions(readWritingSessions(nextProjectId)); setSessionClock(Date.now()); setRepositoryLocation(location);
    }).catch((error) => {
      if (cancelled) return;
      setNotice(`本地作品库未启动，将使用兼容模式：${error instanceof Error ? error.message : "未知错误"}`);
      setRepositoryLocation({ mode: "browser" });
    });
  ...
  const runStoryAgent = async (chapterList: SavedChapter[], trigger: "save" | "timer" | "manual") => {
    ...
    try {
      const analysis = await analyzeSavedChapters(modelConfig, chapterList, storyAnalysis || undefined);
  ...
  const selectProject = (projectId: string) => {
    ...
    setSavedChapters(readSavedChapters(projectId)); setStoryAnalysis(readStoryAnalysis(projectId)); setChapterGuide(readChapterGuide(projectId, project.chapters[0]?.id || 0) || emptyChapterGuide(project.chapters[0]?.id || 0)); setSkills(readSkills(projectId)); setEditorPreferences(readEditorPreferences(projectId)); setActiveWritingSession(readActiveWritingSession(projectId)); setWritingSessions(readWritingSessions(projectId)); setSessionClock(Date.now()); setView("write");
  };
  ...
  const selectChapter = (chapter: Chapter) => {
    ...
    setActiveChapter(chapter.id); setChapterGuide(readChapterGuide(activeProject?.id || "demo-project", chapter.id) || emptyChapterGuide(chapter.id)); setDraft(savedChapters.find((item) => item.id === chapter.id)?.content || (activeProject?.id === "demo-project" && chapter.id === 1 ? initialDraft : "")); setView("write");
  };
  ...
  const saveDraft = async () => {
    ...
    setSaving(true);
    try {
      await saveSavedChapter(activeProject.id, activeProject.title, nextChapter);
      ...
      setNotice("已保存到本地作品库。故事 Agent 正在整理已保存内容。"); void runStoryAgent(nextSaved, "save");
    } catch (error) {
      setNotice(`保存失败，正文仍在编辑器中：${error instanceof Error ? error.message : "未知错误"}`);
  ...
  const createItem = () => {
    ...
    if (creationMode === "project") {
      ...
      const nextProjects = [...projects, project]; setProjects(nextProjects); saveProjects(nextProjects); saveActiveProjectId(project.id); setActiveProjectId(project.id); setActiveChapter(0); setChapterGuide(emptyChapterGuide(0)); setDraft(""); setSavedChapters([]); setStoryAnalysis(null); setSkills([]); setEditorPreferences(readEditorPreferences(project.id)); setActiveWritingSession(null); setWritingSessions([]); setSessionClock(Date.now()); setView("write"); setCreationMode(null); setCreationTitle(""); setNotice(`已新建作品「${title}」，现在添加第一章。`); return;
    }
    if (creationMode === "chapter" && activeProject) {
      ...
      updateChapters([...activeProject.chapters, chapter]); setActiveChapter(chapter.id); setChapterGuide(emptyChapterGuide(chapter.id)); setDraft(""); setCreationMode(null); setCreationTitle(""); setView("write"); setNotice(`已添加第${chapter.id}章「${title}」。`);
    }
  };
